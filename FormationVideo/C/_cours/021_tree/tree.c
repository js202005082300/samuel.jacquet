...
Tree *new_tree(int x)
{
	...
	tr->parent = NULL;
	tr->tleft = NULL;
	tr->tright = NULL; // Pas de parent car c'est la racine (arbre de haut en bas, vue informatique).
...
void print_tree_prefix(Tree *tr)
{
	...
	if(tr->parent != NULL) // Si ce n'est pas la racine.
    	...
    	printf("(%d)\n", tr->value);
	/* #4# Affichage et recursivité.*/
    if(tr->tleft != NULL)
    	print_tree_prefix(tr->tleft);
    if(tr->tright != NULL)
    	print_tree_prefix(tr->tright);
	/* #5# Inverser les deux if pour un postfix.
    if(tr->tleft != NULL)
    	print_tree_prefix(tr->tleft);
    if(tr->tright != NULL)
    	print_tree_prefix(tr->tright);
...
int count_tree_nodes(Tree *tr)
{
	...
	return (count_tree_nodes(tr->tleft) + count_tree_nodes(tr->tright)); // Sans compter la racine (+1).
}
